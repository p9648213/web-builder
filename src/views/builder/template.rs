pub fn render_choose_template() -> maud::Markup {
    maud::html! {
        section {
            div class="relative my-6 flex w-96 flex-col rounded-lg border border-slate-200 bg-white shadow-sm" {
                div class="relative m-2.5 h-56 overflow-hidden rounded-md text-white" {
                    img class="h-full w-full object-cover" src="/assets//images/real_estate.webp" alt="card-image";
                }
                div class="p-4" {
                    div class="mb-2 flex w-full items-center justify-between" {
                        ...
                        a class="group flex items-center gap-2 text-indigo-500" href="/app/demo/realestate" {
                            div class="group" {
                                "Preview"
                            }
                            svg class="flex-0 h-0 w-0 translate-y-[1px] transition-all group-hover:h-4 group-hover:w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" {
